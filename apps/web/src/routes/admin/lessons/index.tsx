...
function LessonsPage() {
	const {
		...
		error: lessonsError,
	} = useQueryWithStatus(api.lessons.listLessons);

	const { data: sections, isPending: isSectionsPending } = useQueryWithStatus(
	...
	const isPending =
		isLessonsPending || isSectionsPending || isChaptersPending || isSubjectsPending;
	...
	if (isLessonsError) {
		return (
			<div className="space-y-6">
				...
				<QueryError error={lessonsError} title="Failed to load lessons" />
			</div>
		);
	}
	...
	const sectionsByChapter = new Map<string, NonNullable<typeof sections>>();
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								{filteredLessons?.length ?? 0} lesson
								{(filteredLessons?.length ?? 0) !== 1 ? 's' : ''}{' '}
										...
										<TableRow key={lesson._id}>
											...
											<TableCell className="text-muted-foreground">
												<div className="flex flex-col">
													<span className="text-xs">{chapter?.name ?? 'Unknown'}</span>
													<span>{section?.name ?? 'Unknown'}</span>
												</div>
											</TableCell>
											<TableCell>
